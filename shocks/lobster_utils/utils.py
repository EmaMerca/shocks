...
import shocks.lobster_utils.lobster_util as lu
...
def load_column_from_ohcl(
    symbols, format_file, column="close", pickled_data=False
) -> pd.DataFrame:
    """
...
def candlestick_from_7z(
    sym_7z: str, startdate: str, lastdate: str, granularity: config.Granularity
) -> None:
    """use plotly to plot candle stick

    the input df should contains: date, open, high, low, close
    """
    lu.from_7z_to_unique_df(
        sym_7z, startdate, lastdate, granularity=granularity, plot=True, level=1
    )
