...
import shocks.lobster_utils.utils as utils
import shocks.lobster_utils.config as config

COLUMNS_NAMES = {
    "orderbook": ["sell", "vsell", "buy", "vbuy"],
...
def orderbook_columns(level: int):
    """return the column names for the LOBSTER orderbook, according to the input level"""
    orderbook_columns = []
...
def ohlc_df_from_7z(
    ...
    plot: bool = False,
    granularity: config.Granularity = config.Granularity.Hour1,
) -> pd.DataFrame:
    """read the input 7z lobster file and return an ohlc df with the selected granularity

    both first_date and last_date are included in the output!!
    """
    message_dfs = read_7z_sub_routine(
        file_7z, first_date, last_date, "message", level=1
    )
...
def unpack_from_7z(
    ...
    """
    message_dfs = read_7z_sub_routine(
        file_7z, first_date, last_date, "message", level=level
    )
    orderbook_dfs = read_7z_sub_routine(
        file_7z, first_date, last_date, "orderbook", level=level
    )
    ...
    messages = []
    orderbooks = []
    assert list(message_dfs.keys()) == list(
        orderbook_dfs.keys()
    ), "the messages and orderbooks have different days!!"
    for d in keys_mess:
        # ADD DATE
...
def read_7z_sub_routine(
    ...
    assert type_file in (
        "orderbook",
        "message",
    ), "The input type_file: {} is not valid".format(type_file)
        ...
        with libarchive.public.memory_reader(buffer_) as e:
            for entry in e:
                ...
                if type_file == "orderbook":
                    ...
                    df = pd.DataFrame(
                        [x.split(",")[: len(columns)] for x in text.split("\n")],
                        columns=columns,
                        dtype="float64",
                    )
                else:
...
def lobster_to_ohlc(
    ...
    df_m_executed = message_df[
        (
            message_df["event_type"].isin(
                [
                    config.OrderEvent.EXECUTION.value,
    ...
    ohlc_df = (
        df_m_executed[["price", "size", "order_id"]]
        .resample(granularity.value)
        .agg({"price": "ohlc", "size": "sum", "order_id": "count"})
    )
...
if __name__ == "__main__":
    ...
    if test_ohlc:
        ohlc = ohlc_df_from_7z(
            "data/_exampledata_AAPL_2018-01-02_2019-06-02_10.7z",
    ...
    test_unpack = True
    if test_unpack:
        orderbook_df, message_df = unpack_from_7z(
            "data/_exampledata_AAPL_2018-01-02_2019-06-02_10.7z",
            level=10,
            first_date="2018-02-05",
            last_date="2018-02-05",
        )
